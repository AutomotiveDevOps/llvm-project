...
DecodeStatus PPCDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
  ...
  // Try VLE 16-bit instruction decoding first if VLE is enabled.
  // VLE instructions have distinct opcodes in the top 5 bits.
  if (STI.getFeatureBits()[PPC::FeatureVLE] && Bytes.size() >= 2) {
    uint16_t Inst16 = ReadFunc16(Bytes.data());
    // Try decoding as VLE 16-bit instruction
