...
DecodeStatus PPCDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
  ...
  // According to AN4648, VLE instructions can be 16-bit or 32-bit.
  // The algorithm checks bits 31 and 28: (instruction & 0x9000) == 0x1000
  // for 32-bit VLE instructions when checking the upper 16 bits.
  if (STI.getFeatureBits()[PPC::FeatureVLE]) {
    ...
    if (Bytes.size() >= 2) {
      ...
      // The decoder tables will be generated by TableGen for VLE instructions
      DecodeStatus result = decodeInstruction(DecoderTableVLE16, MI, Inst16,
                                             Address, this, STI);
      if (result != MCDisassembler::Fail) {
        Size = 2;
        return result;
      ...
      // If 16-bit decode failed and we have 4 bytes, check if it might be
      // a 32-bit VLE instruction using the AN4648 algorithm
      if (Bytes.size() >= 4) {
        uint32_t Inst32 = ReadFunc(Bytes.data());
        // Check if this is a 32-bit VLE instruction according to AN4648
        // Note: We assume VLE context here (could be improved with MMU info)
        if (PPC::isVLE32BitInstruction(Inst32)) {
          // It's a 32-bit VLE instruction, try decoding as such
          // (Note: 32-bit VLE decoder table may not be implemented yet)
          // For now, fall through to standard 32-bit decoding
        }
      }
    }
  }
